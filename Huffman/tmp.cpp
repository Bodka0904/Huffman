...
private:
	...
	void printPreOrder(Node* node)
	{
		...
		{
			if (node->leaf)
				cout << node->code << " " << (char)strtol(node->code.c_str(), 0, 2) << std::endl;
	...
	void createBranch(Node** node, const std::string& path, size_t& index)
	{
		...
		{
			(*node)->leaf = true;
			(*node)->code = path.substr(index + 1, 8);
			(*node)->c = (char)strtol(path.substr(index + 1, 8).c_str(), 0, 2);
			//std::cout << "Creating leaf "<< (*node)->code << " " << (char)strtol((*node)->code.c_str(), 0, 2) << std::endl;
			index += 9;
		}
		return;
	}
...
public:
	...
	bool decompress(const char* inFileName, const char* outFileName)
	{
		...
		size_t counter = m_Tree.CreateBranch(ostr.str());
		...
		if (ostr.str()[counter] == '0')
		{
			m_NumChars = binaryToDecimal(ostr.str().substr(counter + 1, 12));
			//std::cout << m_NumChars << std::endl;
			//std::cout << counter << std::endl;
			//std::cout << ostr.str().size() << std::endl;
			counter += 13;
			while (counter < ostr.str().size())
			{

				if (m_Tree.MoveInTree(ostr.str()[counter], tmp))
				{
		...
		else
		{
			...
			while (counter < ostr.str().size())
			{
				...
				if (ostr.str()[counter] == '0')
				{
					int test = binaryToDecimal(ostr.str().substr(counter + 1, 12));
					if (test == 0)
						break;
					m_NumChars += test;
		...
		//std::cout << "Ouha " << countChars << " " << m_NumChars << std::endl;
		if (countChars != (int)m_NumChars)
			return false;
	...
	bool IsAscii(string c) {
		for (size_t i = 0; i < c.size(); i++)
		{
			if (c.c_str()[i] < 0) return false;
		}
		return true;
	}
